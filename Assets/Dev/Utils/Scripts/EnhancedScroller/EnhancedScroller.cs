...
namespace EnhancedUI.EnhancedScroller {
    ...
    /// </summary>
    /// <param name="scroller">The scroller that changed state</param>
    /// <param name="scrolling">Whether or not the scroller is tweening</param>
    public delegate void ScrollerScrollingDelegate(EnhancedScroller scroller, bool scrolling);
    ...
    #region Public
        ...
        public bool notHideRecyledContainer = false;
        ...
        public ScrollerScrollingDelegate scrollerScrolling;
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab) {
            ...
            if (cellView == null) {
                ...
                var cellRect = cellView.GetComponent<RectTransform>();
                if (cellRect != null) {
                    if (scrollDirection == ScrollDirectionEnum.Vertical) {
                        cellRect.anchorMin = new Vector2(0.5f,                                                 1);
                        cellRect.anchorMax = new Vector2(0.5f,                                                 1);
                        cellRect.sizeDelta = new Vector2(_container.rect.width - padding.left - padding.right, cellRect.sizeDelta.y);
                        cellRect.pivot     = new Vector2(0.5f,                                                 0);
                    }
                    else {
                        cellRect.anchorMin = new Vector2(0,                    0.5f);
                        cellRect.anchorMax = new Vector2(0,                    0.5f);
                        cellRect.sizeDelta = new Vector2(cellRect.sizeDelta.x, _container.rect.height - padding.top - padding.bottom);
                        cellRect.pivot     = new Vector2(0,                    0.5f);
                    }

                    //StartCoroutine(delayAddTempCell(cellPrefab));
                }
    ...
    #region Private
        ...
        private float _AddCellViewSizes() {
            ...
            for (var i = 0; i < NumberOfCells; i++) {
                ...
                _cellViewSizeArray.Add(_delegate.GetCellViewSize(this, i) + (i == 0 ? 0 : (_layoutGroup ? _layoutGroup.spacing : 0)));
        ...
        private void _RecycleCell(EnhancedScrollerCellView cellView) {
            ...
            if (cellView.transform.parent != _recycledCellViewContainer) {
...
//            cellView.transform.localScale = Vector3.zero;            
        ...
        private void _AddCellView(int cellIndex, ListPositionEnum listPosition) {
            ...
            if (cellView.transform.parent != _container) {
...
//                }
                if (scrollDirection == ScrollDirectionEnum.Horizontal) {
                    cellRect.anchoredPosition = new Vector2(_cellViewOffsetArray[cellIndex] - cellRect.sizeDelta.x, 0);
                    cellRect.sizeDelta        = new Vector2(layoutElement.minWidth,                                 cellRect.sizeDelta.y);
                }
                else {
                    cellRect.anchoredPosition = new Vector2(0,                    -_cellViewOffsetArray[cellIndex] - padding.top);
                    cellRect.sizeDelta        = new Vector2(cellRect.sizeDelta.x, layoutElement.minHeight);
                }
            }
        ...
        public void _CalculateCurrentActiveCellRange(out int startIndex, out int endIndex) {
            startIndex = 0;
            endIndex   = 0;
            ...
            startIndex = GetCellViewIndexAtPosition(startPosition) - bufferBefore;
            endIndex   = GetCellViewIndexAtPosition(endPosition) + bufferAfter;

            if (startIndex < 0)
            {
                startIndex = 0;
            }
            if (endIndex > _cellViewOffsetArray.Count - 1)
            {
                endIndex = _cellViewOffsetArray.Count - 1;
            }
        }
        ...
        void Awake() {
            ...
            if (notHideRecyledContainer) {
                _recycledCellViewContainer.gameObject.SetActive(true);
                _recycledCellViewContainer.localScale = Vector3.zero;
            }
            else {
        ...
        void Update() {
            ...
            if (scrollerScrolling != null) { scrollerScrolling(this, true); }
        }
