using Google.FlatBuffers;
using System.Collections;
using System.Collections.Generic;
using System.Drawing;
using CoreData.UniFlow.Common;
using UnityEngine;
using Fbs;
using UnityEngine.UIElements;
using TaskData = CoreData.UniFlow.Common.TaskData;

public class UpdateTaskDataHelper : MonoBehaviour
{
    public static byte[] CreateFbUpdateTaskDataRequestBody(UpdateTaskDataRequestBody body)
    {
        FlatBufferBuilder builder = new(1024);
        var commonTaskData = CreateFbUpdateTaskData(builder, body.TaskDataRequestBody);
        var apiRequest = RequestUpdateTaskData.CreateRequestUpdateTaskData(builder,
            RequestUpdateTaskData.CreateTaskDatasVector(builder,commonTaskData)
            );
        builder.Finish(apiRequest.Value);

        return builder.SizedByteArray();
    }

    private static Offset<Fbs.TaskData>[] CreateFbUpdateTaskData(FlatBufferBuilder builder, List<TaskData> data)
    {
        var itemQueue = new Offset<Fbs.TaskData>[data.Count];
        for (int i = 0; i < data.Count; i++)
        {
            var commonResource = CreateFbCommonResource(builder, data[i].CommonResources);
            var workerPosition = CreateFbWorkerPosition(builder, data[i].PositionWorkerBuildings);
            var vassalPosition = CreateFbVassalPosition(builder, data[i].PositionVassalBuildings);
            Offset<Fbs.TaskData> offset = Fbs.TaskData.CreateTaskData(builder,
                data[i].TaskId,
                data[i].BuildingObjectId,
                data[i].Man,
                data[i].Model,
                data[i].Size,
                builder.CreateString(data[i].Name),
                data[i].Priority,
                data[i].TimeJob,
                builder.CreateString(data[i].Position),
                data[i].TimeBegin,
                data[i].IdJob,
                builder.CreateString(data[i].RotationDoor),
                (int)data[i].StatusFillResource,
                data[i].Workload,
                data[i].IdVassal,
                data[i].BuildingId,
                data[i].SpeedRate,
                data[i].CurrentProductId,
                data[i].WorkloadDone,
                builder.CreateVectorOfTables(commonResource) ,
                builder.CreateVectorOfTables(workerPosition),
                builder.CreateVectorOfTables(vassalPosition)
            );
            itemQueue[i] = offset;
        }
        return itemQueue;

    }

    private static Offset<CurrentItemMaterial>[] CreateFbCommonResource(FlatBufferBuilder builder, List<ResourcePoco> data)
    {
        var itemQueue = new Offset<CurrentItemMaterial>[data.Count];
        for (int i = 0; i < data.Count; i++)
        {
            Offset<CurrentItemMaterial> offset = CurrentItemMaterial.CreateCurrentItemMaterial(builder,
                        data[i].itemId,
                        data[i].itemCount);
            itemQueue[i] = offset;
        }
        return itemQueue;

    }

    private static Offset<WorkerPosition>[] CreateFbWorkerPosition(FlatBufferBuilder builder, List<PositionWorkerBuilding> data)
    {
        var itemQueue = new Offset<WorkerPosition>[data.Count];
        for (int i = 0; i < data.Count; i++)
        {
            var pos = CreateFbWorkerVector(builder, data[i].Position);
            Offset<WorkerPosition> offset = WorkerPosition.CreateWorkerPosition(builder,
                        data[i].Index,
                        pos,
                        data[i].IsEmpty,
                        data[i].RotationY);
            itemQueue[i] = offset;


        }
        return itemQueue;
    }


    private static Offset<WorkerPosition>[] CreateFbVassalPosition(FlatBufferBuilder builder, List<PositionWorkerBuilding> data)
    {
        var itemQueue = new Offset<WorkerPosition>[data.Count];
        for (int i = 0; i < data.Count; i++)
        {
            var pos = CreateFbWorkerVector(builder, data[i].Position);
            Offset<WorkerPosition> offset = WorkerPosition.CreateWorkerPosition(builder,
                        data[i].Index,
                        pos,
                        data[i].IsEmpty,
                        data[i].RotationY);
            itemQueue[i] = offset;


        }
        return itemQueue;
    }

    private static Offset<Fbs.Vector3> CreateFbWorkerVector(FlatBufferBuilder builder, UnityEngine.Vector3 position)
    {
        Offset<Fbs.Vector3> offset = Fbs.Vector3.CreateVector3(builder,
                         position.x,
                         position.y,
                         position.z);
        return offset;
    }
}

public class UpdateTaskDataRequestBody
{
    public List<TaskData> TaskDataRequestBody;
}

public class TaskDataRequestBody
{
    public int IdTaskData;
    public int BuildingObjectId;
    public int Worker;
    public int Model;
    public float Size;
    public string Name;
    public int Priority;
    public float Timetask;
    public string Position;
    public float TimeBegin;
    public int IdJob;
    public string RotationDoor;
    public int StatusFillResource;
    public float Workload;
    public int IdVassal;
    public int BuildingId;
    public float SpeedRate;
    public int ProductId;
    public List<ResourcePoco> CommonResource;
    public List<PositionWorkerBuilding> WorkerPossition;
    public List<PositionWorkerBuilding> VassalPossition;
}