...
{
    public class CollectResource : MonoSingleton<CollectResource>
    {
        public void OnStartCollectResource(Vector3 sizetilingTransform, Vector3 posTilingTransform, List<ResourcePoco> _commonResources, UserBuilding userBuilding, int buildingObjectId)
        {
            int porter = 3;
            for (int i = 0; i < porter; i++)
            {
                if (i < Utils.IsBringResource(_commonResources))
                {
                    ...
                    DelayWorkerCollect(i, porterAnim, sizetilingTransform, posTilingTransform, _commonResources, userBuilding, buildingObjectId);
                }
            }
        }

        private async void DelayWorkerCollect(int index, CharacterTaskData Assistan, Vector3 sizetilingTransform,
           Vector3 posTilingTransform, List<ResourcePoco> _commonResources, UserBuilding userBuilding, int buildingObjectId)
        {
            ...
            porterGoesToHarvest(index, Assistan, sizetilingTransform, posTilingTransform, _commonResources, userBuilding, buildingObjectId);
        }

        public void porterGoesToHarvest(int index, CharacterTaskData Assistan, Vector3 sizetilingTransform,
           Vector3 posTilingTransform, List<ResourcePoco> _commonResources, UserBuilding userBuilding, int buildingObjectId)
        {
                    ...
                    Assistan.CharacterBehaviour.MoveCenter(Vector3.zero, 2f, posTilingTransform, (dataStore, rotation) =>
                    {
                        ...
                        Assistan.CharacterBehaviour.MoveCenter(Vector3.zero, 1.5f, posOutDoorHouse, (dataStore, rotation) =>
                        {
                            ...
                            if (countAmount.Count == 0)
                            {
                                ...
                                {
                                    VassalFinishDestroy(buildingObjectId);
        ...
        public void VassalFinishDestroy(int buildingObjectId)
        {
